...
while (my $line = <IN_FILE>) { 
	...
	$row->{text}  = $6;
        ...
        if ($row->{file} ne $prev_file or eof IN_FILE) {
            ...
            print 'found: ' . scalar @words . ' words: ' . scalar @sampas . ' sampas, ' . scalar @cvs . " cvs\n";
	    if (scalar @cvs != scalar @sampas) {
		...
		# now print the word and its cvs and sampas to the sql file
                print SQL_FILE "$id\t$prev_file\t$word{text}\t$word{xmin}\t$word{xmax}\t$sampa_text\t$cv_text\n";
                $id++;
                $cv_text = '';
                $sampa_text = '';
